  ...
  try {
    ...
    await db.run(
      `INSERT INTO users (id, email, password, name, subdomain, created_at)
       VALUES (?, ?, ?, ?, ?, datetime('now'))`,
      [userId, email, hashedPassword, name, subdomain]
    );
    ...
    res.json({
      ...
      token,
      message: `欢迎! 你的简历将在以下地址展示: ${subdomain}.resumeBuilder.com`
    });
  } catch (error) {
    console.error(error);
...
app.post('/api/resume/generate', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      publicUrl: `https://${user.subdomain}.resumeBuilder.com`
    });
  } catch (error) {
    console.error(error);
...
app.get('/preview/:resumeId', async (req, res) => {
  try {
    ...
    const resumeData = {
      ...
      education: JSON.parse(resume.education),
      skills: JSON.parse(resume.skills),
      zodiac: JSON.parse(resume.zodiac)
    };
